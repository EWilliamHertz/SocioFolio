// src/app/dashboard/create/CreateResumeForm.tsx
"use client";
...
export default function CreateResumeForm() {
  ...
  const [customSections, setCustomSections] = useState([{ title: "", content: "" }]);
  ...
  return (
    <form action={createResume} className="space-y-8">
      {/* Hidden fields to pass complex data to the server action */}
      ...
      <div className="space-y-4">
        <div>
          ...
          <input type="text" name="title" required placeholder="e.g. Graphic Designer Portfolio" className="mt-1 block w-full px-4 py-2 border border-neutral-300 rounded-md focus:ring-blue-500 focus:border-blue-500" />
        </div>
        
        <div>
          <label className="block text-sm font-medium text-neutral-700">Short Summary</label>
          <textarea name="summary" rows={2} required placeholder="A brief hook about you..." className="mt-1 block w-full px-4 py-2 border border-neutral-300 rounded-md focus:ring-blue-500 focus:border-blue-500"></textarea>
        </div>
      </div>
      ...
      <div className="p-6 bg-neutral-50 rounded-lg border border-neutral-200 space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-neutral-700">YouTube Video URL (Optional)</label>
          <input type="url" name="youtubeUrl" placeholder="https://youtube.com/watch?v=..." className="mt-1 block w-full px-4 py-2 border border-neutral-300 rounded-md focus:ring-blue-500 focus:border-blue-500" />
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        {customSections.map((section, index) => (
          <div key={index} className="p-4 border border-neutral-200 rounded-md bg-white space-y-3 shadow-sm">
            <input 
              type="text" 
              placeholder="Section Name (e.g., Work Experience, Skills)" 
              value={section.title} 
      ...
      <div className="flex items-center pt-4 border-t border-neutral-100">
        <input id="isHighlighted" name="isHighlighted" type="checkbox" className="h-4 w-4 text-black border-neutral-300 rounded" />
